...
if __name__=="__main__":
	...
	hydro_actual_a_pressure = ReadMemory(plc,20,0,S7WLReal)
        hydro_actual_b_pressure = 'NA'#ReadMemory(plc,24,0,S7WLReal)
        hydraulic_actual_pressure =ReadMemory(plc,28,0,S7WLReal)
        cycle_status= ReadMemory(plc,76,0,S7WLWord)
        stabilization_timer= 'NA'#ReadMemory(plc,48,0,S7WLWord)
        holding_timer= 'NA'#ReadMemory(plc,50,0,S7WLWord)
        drain_timer= 'NA'#ReadMemory(plc,60,0,S7WLWord)
        machine_mode =ReadMemory(plc,2,0,S7WLWord)
        offline_online = '0'
        pop_ups = 'NA'#ReadMemory(plc,18,0,S7WLWord)
        invalid = 'NA'#ReadMemory(plc,0,4,S7WLBit)
        result = 'NA'#ReadMemory(plc,78,0,S7WLWord)
        bubble = 'NA'#ReadMemory(plc,108,0,S7WLWord)
